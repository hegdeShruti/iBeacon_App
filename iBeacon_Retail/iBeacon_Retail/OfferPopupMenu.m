...
#pragma mark OfferPopupMenuItem

@implementation OfferPopupMenuItem
...
#pragma mark OfferPopupMenuItemView

@interface OfferPopupMenuItemView : UIView
...
@implementation OfferPopupMenuItemView


-(instancetype)initWithMenuItem:(OfferPopupMenuItem*)item frame:(CGRect)frame menuStyle:(PopoutMenuStyle)menuStyle{
    if (self = [super init]) {
...
#pragma mark OfferPopupMenu

@interface OfferPopupMenu ()
...
@property (nonatomic)OfferPopupMenuItemView * selectedItemView;
@property (nonatomic)OfferPopupMenuItem * selectedItem;

@end

@implementation OfferPopupMenu
...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message{
    return [self initWithTitle:title message:message items:nil];
}

-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message images:(NSArray *)images{
    NSMutableArray * items = [NSMutableArray new];
    [images enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        OfferPopupMenuItem * item = [[OfferPopupMenuItem alloc]initWithTitle:nil image:(UIImage*)obj];
...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message itemTitles:(NSArray *)itemTitles{
    ...
    [itemTitles enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        OfferPopupMenuItem * item = [[OfferPopupMenuItem alloc]initWithTitle:(NSString*)obj image:nil];
        [items addObject:item];
    }];
    return [self initWithTitle:title message:message items:items];
}
...
-(OfferPopupMenuItemView*)itemViewatPoint:(CGPoint)point{
    OfferPopupMenuItemView * selectedItemView = nil;
    OfferPopupMenuItem * selectedItem = nil;
    if (CGRectContainsPoint(self.menuView.frame, point)) {
        point = [self.view convertPoint:point toView:self.menuView];
        selectedItemView = [self.itemViews filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(OfferPopupMenuItemView * itemView, NSDictionary *bindings) {
...
-(void)layoutasDefault{
    ...
    [self.items enumerateObjectsUsingBlock:^(OfferPopupMenuItem * obj, NSUInteger idx, BOOL *stop) {
        NSUInteger index = idx;
        while (index >= colCount) {
            index -= colCount;
        }
        NSUInteger rowIndex = floorf((CGFloat)idx/colCount);
        OfferPopupMenuItemView * itemView = nil;
        if (idx >= [self.items count]-([self.items count]%colCount)) {
            ...
            itemView = [[OfferPopupMenuItemView alloc]initWithMenuItem:obj
...
-(void)layoutasList{
    ...
    [self.items enumerateObjectsUsingBlock:^(OfferPopupMenuItem*obj, NSUInteger idx, BOOL *stop) {
        OfferPopupMenuItemView * itemView = [[OfferPopupMenuItemView alloc]initWithMenuItem:obj frame:CGRectMake(offSetX, offSetY+idx*itemSize.height, itemSize.width, itemSize.height) menuStyle:self.menuStyle];
...
-(void)layoutasOval{
    ...
    [self.items enumerateObjectsUsingBlock:^(OfferPopupMenuItem * obj, NSUInteger idx, BOOL *stop) {
        ...
        OfferPopupMenuItemView * itemView = [[OfferPopupMenuItemView alloc]initWithMenuItem:obj frame:CGRectMake(0, 0, itemSize.width, itemSize.height) menuStyle:self.menuStyle];
